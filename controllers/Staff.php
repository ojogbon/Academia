...
function insertStaff($staff,$key, $firstName,$lastName,$password,$confirm_password,$passport,$email,$staff_id,$phonenumber,$address,$gender,$type){
            ...
            echo "<div class='alert alert-danger'><b>Fields can't be empty!</b>   Please fill and try again</div>";
              ...
              if ( $uploaded[0] !== "TRUE" ){
                ...
                if ($password != $confirm_password ){
                    ...
                    if($staff->saveStaff("`staff_tbl`(`id`, `staff_id`, `firstname`, `lastname`,
                     `email`, `password`, `phonenumber`, `address`, `gender`, `type`, `image`, `path`, `date`, `status`, `deleted`)",
                        "VALUES (null,'$staff_id','$firstName','$lastName', '$email','$password','$phonenumber'
                        ,'$address','$gender','$type','$uploaded[1]','$uploaded[2]'
                        ,now(),'0','0')")){
